...
public class SearchWebHandler extends AbstractHandler {
    ...
    public void handleDocument(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        writer.write(("<HTML><TITLE>StackOverflow</TITLE><HEAD><H3>" + document.metadata.get("title") + "</H3></HEAD>"));
        writer.write(("<H3>" + document.metadata.get("question") + "</H3>"));
        writer.write(("<BODY><p>" + document.metadata.get("answers") + "</p>"));
        writer.write(("<p>" + document.metadata.get("comments")  + "</p></BODY></HTML>"));
        writer.close();
    }
    ...
    public void handleSearch(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        System.out.println("Time elapsed for this query: " +  elapsedTimeMillis/1000F);
        ...
        for (SearchResultItem item : result.items) {
            ...
            //System.out.println("item.medtadata" + item.metadata.isEmpty());
            ...
            writer.append(String.format("<a href=\"document?identifier=%s\">%s</a><br/>" +
                                        ...
                                        item.summary,
                                        scrub("" + (String.format("%10.8f", item.score)))
                                        ));
            } else
                 writer.append(String.format("<a href=\"document?identifier=%s\">%s</a><br/>" +
                                        //"<div id=\"summary\">%s</div>\n" +
    ...
    private SearchResult performSearch(HttpServletRequest request) throws Exception {
       ...
       //  System.out.println("kuku is a good boy.");
        SearchResult result = search.runQuery(query, startAt, resultCount, false); // Kisi chutiye ne kuch kiya yaha
        //System.out.println("kuku loves sukh." + result.items.size());
