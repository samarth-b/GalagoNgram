...
public class App {
    ...
    private void handleBuildngram(String[] args) throws Exception {
        // Remove 'build' from the command.
        args = Utility.subarray(args, 1);
        ...
        String[] flags = filtered[0];
        String[] nonFlags = filtered[1];
        int ngram = Integer.parseInt(nonFlags[0]);
        ...
        //System.out.println("The nonFlags argument =" + ngram);
        
        for(int i =1; i <= ngram; i++)
        {
            System.out.println("Creating Index for Ngram = " + i);
            BuildIndex build = new BuildIndex(i);
            Job job = build.getIndexJob(indexName + i, docs, useLinks, stemming);
            ErrorStore store = new ErrorStore();
            JobExecutor.runLocally(job, store, keepOutput);
            if (store.hasStatements()) {
                output.println(store.toString());
    ...
    private void handleDumpCorpus(String[] args) throws IOException {
        ...
        while (!iterator.isDone()) {
            ...
            output.println("#MY IDENTIFIER: " + document.identifier);
...
//            }
            ...
            output.println(document.metadata.get("title"));
            output.println("#QUESTION");
            output.println(document.metadata.get("question"));
            output.println("#ANSWERS");
            output.println(document.metadata.get("answers"));
            
            output.println("#COMMENTS");
            output.println(document.metadata.get("comments"));
    ...
    private void handleMakeCorpus(String[] args) throws Exception {
     ...
     private void handleBatchNgramSearch(String[] args) throws Exception {
        if (args.length <= 1) {
            commandHelpBatchSearch();
            return;
        }

        BatchSearch.runNgram(Utility.subarray(args, 1), output);
    }
    ...
    private void handleSearch(Retrieval retrieval, DocumentStore store) throws Exception {
        ...
        private void handleSearchNgrams(Retrieval[] retrieval,int n, DocumentStore store) throws Exception {
        Search search = new Search(retrieval, n, store);
        int port = Utility.getFreePort();
        Server server = new Server(port);
        server.addHandler(new SearchWebHandler(search));
        server.start();
    ...
    private void handleSearchngrams(String[] args) throws Exception {
        if (args.length <= 1) {
            commandHelp("search");
            return;
        }
        ...
        String[][] filtered = Utility.filterFlags(Utility.subarray(args, 3));
        String[] flags = filtered[0];
        String[] corpusFiles = filtered[1];
        
        System.out.println("corpus:" + corpusFiles[0]);
//        ngram = 2;
//        corpusFiles[0] = new String("./my.index");
//        indexPath[0] = new String("./mycorp.corpus");
        
        System.out.println("Choosen param:" + ngram);
        // Any flag marked '--parameters' marks a parameters file.
        // We trim that part of the flag off so that the Parameters object will
        // load it as a parameters file.
        for (int i = 0; i < flags.length; ++i) {
        ...
        Retrieval[] retrievals = new Retrieval[ngram];
        for(int i =1; i <= ngram; i++)
            retrievals[i-1] = Retrieval.instance(indexPath + i, p);
        
        System.out.println("I created Retrievals:" + ngram);
        handleSearchNgrams(retrievals, ngram, getDocumentStore(corpusFiles));
    }   
    ...
    public void usage() {
        ...
        output.println("   buildngram");
        output.println("   search");
        output.println("   searchngram");
        output.println("   batch-search");
        output.println("   batchngram-search");
    ...
    public void run(String[] args) throws Exception {
        ...
        if (command.equals("help") && args.length > 1) {
        ...
        } else if (command.equals("batchngram-search")) {
            handleBatchNgramSearch(args);
        } else if (command.equals("build")) {
            handleBuild(args);
        } else if (command.equals("buildngram")) {
            handleBuildngram(args);
        } else if (command.equals("doc")) {
            handleDoc(args);
        } else if (command.equals("dump-connection")) {
            ...
            handleSearch(args);
        } else if (command.equals("searchngram")) {
            handleSearchngrams(args);
        } else if (command.equals("eval")) {
            handleEval(args);
        } else {
            usage();
